...
function ensureProductPdfColumns(db, headers) {
  const existing = new Set(db.prepare("PRAGMA table_info(products)").all().map((row) => row.name));
...
function buildProductUpsert(db, headers) {
...
async function main() {
  ...
  const tx = db.transaction(() => {
    const existingProductIds = new Map(
      db.prepare("SELECT reference, id FROM products").all().map((row) => [row.reference, row.id])
    );
    ...
    for (const p of products) {
      ...
      const values = [
        ...
        String(p["Descr.Prod."] ?? "").trim() || reference,
